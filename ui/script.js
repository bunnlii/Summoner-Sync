 (function(){
  ...
  function getChipCount(){ return playersChips.querySelectorAll('.player-chip').length; }
  function updateAddButton(){
    const count = getChipCount();
    addBtn.disabled = count >= maxPlayers;
    addBtn.textContent = count >= maxPlayers ? 'Max players added' : 'Add Another Player';
  }
  ...
  const existing = Array.from(playersChips.querySelectorAll('.chip-name')).map(s => s.textContent.toLowerCase());
  if (existing.includes(val.toLowerCase())) { input.value = ''; input.focus(); return; }
  ...
  searchBtn.addEventListener('click', (e) => {
    ...
    if (errorEl) errorEl.textContent = '';
    hidePopup();
    const chips = Array.from(playersChips.querySelectorAll('.chip-name')).map(s => s.textContent);
    ...
    alert('Searching for: ' + (values.length ? values.join(', ') : '(no names entered)'));
  });
  ...
  function showPopup(message){
    ...
    if (!popup) {
      ...
      popup.innerHTML = '<span class="popup-message" aria-atomic="true"></span><button class="popup-close" aria-label="Dismiss notification">\u00D7</button>';
      ...
      if (closeBtn) {
        ...
        closeBtn.addEventListener('keydown', (e) => {
          if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); hidePopup(); }
        });
      }
    }
    const msg = popup.querySelector('.popup-message');
    if (msg) msg.textContent = message; else popup.textContent = message;
    popup.classList.add('visible');
    popup.setAttribute('aria-hidden','false');
    if (popupTimeout) clearTimeout(popupTimeout);
    popupTimeout = setTimeout(() => { hidePopup(); }, 10000); // 10s
  }
  ...
  function closeAllTooltips(){
    ...
    document.querySelectorAll('.help-icon').forEach(h => h.setAttribute('aria-expanded', 'false'));
  }
  ...
  if (helpIcon) {
    ...
    helpIcon.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleTooltipFor(helpIcon); }
      if (e.key === 'Escape') { closeAllTooltips(); helpIcon.blur(); }
    });
  }
